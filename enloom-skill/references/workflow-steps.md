...
The laws constrain the lifecycle, not individual operations. All five carry through the lifecycle:
...
**Compaction check** — mandatory gate, run after every integrate:
...
2. **Periodic drift detection** (retained role) — triggered in:
   ...
   - **Full tier here**: the complete nine-item scan below runs at Orient entry and at periodic Verify drift checks — *not* at every stage transition. This is where the attention budget for a full read is justified.

### The nine checks (full tier — run at Orient + periodic Verify, not at every transition)

Run read-only checks for:
- **gate files present** at each stage crossing (the landing gates — primary check)

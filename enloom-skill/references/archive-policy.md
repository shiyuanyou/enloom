...
- [ ] **每个 task 的 report Review Result 段已填**(铁律 5 机械化:archive 前 health-check 硬验此条——见 [landing-contract.md](landing-contract.md) §3)。无 Review Result 段 → 不准 archive。
...
动作:把目录从 `.enloom/<created>-<project>/` 移到 `.enloom/archive/<created>-<project>/`。task_board 行不动(已标 `closed`,`project` 列用名字索引,路径变化不影响查找)。

执行方式:派 sub-agent,只做 mv。Fold 是**显式操作**,不与 phase-close 自动绑定——phase 标 `closed` 只是标记,fold 是独立的 workspace-hygiene 动作,在 Stage 0 Triage 进新任务前做(详见 [workflow-steps.md](workflow-steps.md) §Stage 0)。

Fold 不破坏 reopen:`project_state` / tasks / runs / archive 内部结构完整保留;要 reopen → `git mv` 回顶层 + task_board `phase` 改回。

#!/usr/bin/env bash
# description-only trigger eval runner.
#
...
# log to AgentOS/runs/trigger-eval/raw/<split>/<id>.log
set -euo pipefail

# resolve project root: agentos/ (script lives in agentos/AgentOS/runs/trigger-eval/)
PROJ_ROOT="$(cd "$(dirname "$0")/../../.." && pwd)"
cd "$PROJ_ROOT"

SKILL_MD="agentos-workflow-skill/SKILL.md"
EVALS_JSON="agentos-workflow-skill/evals/trigger-evals.json"
RAW_DIR="AgentOS/runs/trigger-eval/raw"

# --- extract description (frontmatter `description:` single line, YAML) ---
SKILL_NAME="agentos-workflow"
